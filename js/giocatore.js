...
const GiocatoreMode = {
    ...
    generaOfferteContratto(overall) {
        ...
        // Determina le leghe compatibili in base all'overall del giocatore
        let legheDisponibili = this.ottieniLeghePerOverall(overall);

        legheDisponibili.forEach(legaKey => {
            // Mappatura formattata per il nome del campionato da mostrare
            let nomeLegaEsteso = legaKey;
            if (legaKey === "serie_a") nomeLegaEsteso = "Serie A";
            else if (legaKey === "serie_b") nomeLegaEsteso = "Serie B";
            else if (legaKey.startsWith("serie_c")) nomeLegaEsteso = "Serie C";
            else if (legaKey.startsWith("serie_d")) nomeLegaEsteso = "Serie D";

            // 1. Recupera le squadre reali per la lega da datiSquadreInterni
            let squadreReali = this.datiSquadreInterni[legaKey] || [];
            
            // 2. Se vuoto, cerca il fallback nel motore Engine.leagues
            if (squadreReali.length === 0 && typeof Engine !== 'undefined' && Engine.leagues) {
                let legaObj = Engine.leagues.find(l => l.key === legaKey || l.nome === legaKey);
                if (legaObj && legaObj.squadre) {
                    squadreReali = legaObj.squadre.map(s => typeof s === 'string' ? s : s.nome);
                }
            }

            // 3. Se sono presenti squadre reali per la lega
            if (squadreReali.length > 0) {
                // Esclude la squadra attuale del giocatore per proporre solo nuovi club
                let squadreDisponibili = squadreReali.filter(s => s.toLowerCase() !== (this.profile.squadra || "").toLowerCase());
                if (squadreDisponibili.length === 0) squadreDisponibili = squadreReali;

                // Seleziona una squadra reale a caso dal campionato
                let squadraSelezionata = squadreDisponibili[Math.floor(Math.random() * squadreDisponibili.length)];
                
                // Calcola lo stipendio proporzionato all'overall e al livello del campionato
                let stipendio = this.calcolaStipendioOfferta(overall, nomeLegaEsteso);
    ...
    calcolaStipendioOfferta(overall, legaKey) {
        // Moltiplicatore base della lega (stipendio medio della categoria)
        const moltiplicatoriLega = {
            "Serie A": 1000000,    // Base 1M€
            "Serie B": 180000,     // Base 180k€
            "Serie C": 45000,      // Base 45k€
            "Serie D": 12000,      // Base 12k€
            "Eccellenza": 4000     // Base 4k€
        };

        // Prende il valore base della lega, con fallback di default se la lega non è in elenco
        let baseLega = moltiplicatoriLega[legaKey] || 10000;

        // Fattore overall: scala esponenzialmente in base al livello del giocatore
        let fattoreOverall = Math.pow(overall / 60, 3.2);

        // Variazione casuale del +/- 15% per simulare la generosità o la disponibilità del club
        let variazioneCasuale = 0.85 + (Math.random() * 0.30);

        // Calcolo finale dello stipendio annuo
        let stipendioAnnuo = Math.round(baseLega * fattoreOverall * variazioneCasuale);

        // Arrotonda la cifra alle centinaia o migliaia più vicine per pulizia visiva
        if (stipendioAnnuo > 100000) {
            stipendioAnnuo = Math.round(stipendioAnnuo / 10000) * 10000;
        } else {
            stipendioAnnuo = Math.round(stipendioAnnuo / 500) * 500;
        }

        return stipendioAnnuo;
    },

    ottieniLeghePerOverall(overall) {
        let leghe = [];

        // Mappatura delle sole leghe italiane in base alla valutazione (overall)
        if (overall >= 78) {
            leghe = ["serie_a"];
        } else if (overall >= 70) {
            leghe = ["serie_a", "serie_b"];
        } else if (overall >= 62) {
            leghe = ["serie_b", "serie_c_a"];
        } else if (overall >= 55) {
            leghe = ["serie_c_a", "serie_d_a"];
        } else {
            leghe = ["serie_d_a", "eccellenza"];
        }

        // Filtra restituendo solo le leghe effettivamente definite nel tuo database di gioco
        return leghe.filter(legaKey => {
            return (this.datiSquadreInterni && this.datiSquadreInterni[legaKey]) ||
                (typeof Engine !== 'undefined' && Engine.leagues && Engine.leagues.some(l => l.key === legaKey || l.nome === legaKey));
        });
    },
    ...
    mostraModalOfferteContratto(offerte, verdetto) {
        ...
        let htmlOfferte = "";
        if (offerte.length === 0) {
            htmlOfferte = `<div class="col-span-full text-center text-xs text-slate-400 py-4">Nessuna offerta di trasferimento ricevuta.</div>`;
        } else {
            offerte.forEach((off) => {
                let stipendioFormattato = typeof off.stipendio === 'number' ? off.stipendio.toLocaleString('it-IT') : off.stipendio;
                htmlOfferte += `
                    <div class="bg-slate-950 border border-slate-800 p-4 rounded-xl flex flex-col justify-between space-y-3 hover:border-amber-500/50 transition-all">
                        ...
                        <div class="text-xs text-slate-400 space-y-1">
                            <p>Ingaggio offerto: <span class="text-emerald-400 font-extrabold">€ ${stipendioFormattato} / anno</span></p>
                        </div>
                        <button onclick="GiocatoreMode.firmaContratto('${off.squadra.replace(/'/g, "\\'")}', '${off.lega}')" class="w-full py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-lg text-xs uppercase transition-colors">
                            Firma Contratto
                        </button>
                    </div>
                `;
